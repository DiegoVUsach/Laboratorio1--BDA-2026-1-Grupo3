...
public class ClanesService {
    ...
    public void delete(String usernameAuth, Integer idClan) {
        ...
        if (clan.getIdLider() == null) {
            throw new RuntimeException("El clan no tiene lider asignado.");
        }
        if (!personajeRepository.perteneceAUsuario(clan.getIdLider(), usernameAuth)) {
            throw new RuntimeException("Solo el lider del clan puede eliminarlo.");
        }
        // Desasociar el lider antes de borrar para no violar FK circular
        clan.setIdLider(null);
        clanesRepository.update(clan);
        clanesRepository.deleteById(idClan);
    }
